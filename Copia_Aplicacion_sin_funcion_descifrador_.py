        ...
        if identificador_principio == b"<IMAGEN>":
            ...
            with open('carpeta_del_cliente/imagen_recibida_'+message[24:]+'.png', 'wb') as file:
                ...
                if identificador_final == b'<FIN>':
                    ...
                    KEY_descifrada_licencia = aesDecryptor.update(KEY_cifrada_licencia) + aesDecryptor.finalize()
                    #no hace falta padding porque tiene longitud 32

                    #print(KEY_descifrada)
                    ...
                    aesCipher = Cipher(algorithms.AES(KEY_descifrada_licencia), modes.CBC(iv))
                    aesDecryptor = aesCipher.decryptor()

                    # Descifrar el contenido
                    mensaje_descifrado = aesDecryptor.update(file_bytes) + aesDecryptor.finalize()

                    # Eliminar el padding PKCS7
                    unpadder = padding.PKCS7(128).unpadder()
                    img_despadding = unpadder.update(mensaje_descifrado) + unpadder.finalize()
            ...
            # Descifrar el contenido
            mensaje_descifrado = aesDecryptor.update(data) + aesDecryptor.finalize()
